...
import { getCachedHappyHourDebugText, getCachedHappyHourFeed } from "./happy-hour.cache.js";
  ...
  server.get<{ Querystring: { debug?: string; filter?: string | string[] } }>(
    ...
    async (request, reply) => {
      if (isDebugRequest(request.query.debug)) {
        const debugText = getCachedHappyHourDebugText(HAPPY_HOUR_SOURCE, request.query.filter);

        return reply.type("text/plain; charset=utf-8").send(debugText);
      }

      const feed = getCachedHappyHourFeed(HAPPY_HOUR_SOURCE, request.query.filter);

      if (!feed) {
        return reply.code(503).type("text/plain; charset=utf-8").send("Happy hour cache warming");
      }
